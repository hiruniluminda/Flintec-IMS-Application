    ...
    $.ajax({
        ...
        success: function (response) {
            if (!response || !Array.isArray(response) || response.length === 0) {
                console.error("Empty or invalid data received from server.");
                return;
            }
            console.log("Server response:", response);
            data_obj = response.map(item => ({
                issue: item.issue || "Unknown Issue",
                description: item.description || "No description"
            }));
            console.log("Data loaded into data_obj:", data_obj);
            loadTablePage(1); // Ensure table is rendered after data is loaded.
        },
        error: function () {
...
function initializeRawData() {
    $.ajax({
        type: "POST",
        dataType: 'json',
        url: url_loadIssueList, // Ensure this URL provides the data needed.
        success: function (response) {
            console.log("Initializing raw_data with server response:", response);

            if (!response || !Array.isArray(response)) {
                console.error("Invalid or empty response for raw_data.");
                return;
            }

            raw_data = response;
            filterTableData(); // Now we can filter the raw_data.
        },
        error: function () {
            console.error("Failed to initialize raw_data.");
        },
    });
}
...
function filterTableData() {
    data_obj.splice(0, data_obj.length); // Clear the existing data.

    if (!raw_data || raw_data.length === 0) {
        console.warn("Raw data is not initialized or is empty.");
        return;
    }

    console.log("Starting data filtering...");

    // Filter raw_data to exclude entries missing essential fields.
    const filteredData = raw_data.filter(item => {
        return item.issue && item.line_name && item.issue_date;
    });

    if (filteredData.length === 0) {
        console.warn("No data passed the filter criteria.");
        return;
    }

    console.log("Initial filter applied. Proceeding to further filtering...");

    var maxIterations = 1000; // Safeguard for infinite loops.
    var iterationCount = 0;

    filteredData.forEach(function (i) {
        if (++iterationCount > maxIterations) {
            console.error("Infinite loop detected in filterTableData.");
            return; // Stop execution if loop limit is exceeded.
        }

        var condition = false;
        var dateVar = i.issue_date.split('(')[1].split(')')[0];

        var dateCheck_result = dateCheck(
            new Date(Unix_timestamp(dateVar, 'ymd')),
            new Date(Date_select_start),
            new Date(Date_select_end)
        );
                ...
                for (var z = 0; z < var_array.length; z++) {
                    for (var q = 0; q < var_array.length; q++) {
                        for (var u = 0; u < var_array.length; u++) {
                            iterationCount++;
                            if (iterationCount > maxIterations) {
                                console.error("Maximum iteration limit reached. Exiting loop.");
                                break OuterLoop;
                            }

                            var binary_code = var_array[x] + var_array[y] + var_array[z] + var_array[q] + var_array[u];

                            condition = (binary_code == comparing_string) && dateCheck_result;

                            if (condition) {
                                var tempArr = [];
                                tempArr.push(Unix_timestamp(dateVar, 'ymd'));
                                tempArr.push(i.issue_occurrence_id);
                                tempArr.push(i.issue);
                                tempArr.push(i.line_name);
                                tempArr.push(i.issue_satus === '1' ? 'Unsolved' : 'Solved');
    ...
    console.log("Data filtering complete:", data_obj.length, "records passed filters.");
...
function loadTablePage(page) {
    // Ensure data_obj is not empty
    if (!data_obj || data_obj.length === 0) {
        console.error("No data available for table rendering.");
        $("#accordion").html("<p style='color:#d1d1d1; text-align:center;'>No Issues Available</p>");
        return;
    }
    ...
    if (isNaN(number_of_pages) || number_of_pages <= 0) {
        console.error("Invalid number of pages calculated.");
        return;
    }

    console.log("Rendering page", page);

    var starting_index = (items_per_page * (page - 1));
    var last_index = items_per_page * page;

    if (last_index > data_obj.length) {
        last_index = data_obj.length;
    }

    // Stop execution if no rows exist to render
    if (starting_index >= last_index) {
        console.warn("No rows to render in the specified range.");
        return;
    }
    ...
    $.ajax({
        ...
        success: function (response) {
            if (!response || response.length === 0) {
                ...
                document.getElementById("columnchart_values").innerHTML = "No data available.";
                return;
            }

            var chartData = [["Element", "Frequency", { role: "style" }]];
            response.forEach(element => {
                chartData.push([element.machine_machine_id, element.count, getRandomColor()]);
            });
            ...
            try {
                ...
                console.error("Error drawing chart:", e.message);
            }
        },
        error: function () {
...
function drawChart2() {
    ...
    $.ajax({
        ...
        success: function (response) {
            if (!response || response.length === 0) {
                document.getElementById("columnchart_values1").innerHTML = "No data available.";
                return;
            }

            // Prepare data for Google Charts
            var chartData = [["Search Description", "Frequency", { role: "style" }]];
            response.forEach(element => {
                chartData.push([element.Search_Description, element.count, getRandomColor()]);
            });
            ...
            var options = {
                bar: { groupWidth: "75%" },
                legend: { position: "none" },
                chartArea: { width: '80%', height: '80%' }, // Increased height
                height: 400, // Set a fixed height
                tooltip: { isHtml: true, trigger: 'focus' }, // Fix tooltip behavior
                animation: { duration: 1000, easing: 'out', startup: true }
            };
...
function drawChart3() {
    ...
    if (dataArray2 != null && dataArray2.length > 0) {
        var handler = setInterval(function () {
            // Safeguard to ensure valid data and progress
            if (percent > 74) {
        ...
        console.error("Data is empty or not properly initialized.");
        // Prevent chart from being drawn with no data
    }
...
function createTable() {
    ...
    var plantLocation = $('#plantSelectBox').val();

    if (!startDate || !endDate || !plantLocation || plantLocation.length === 0) {
        console.warn("Inputs missing. Table will not render.");
        $("#myTable").html("<tr><td colspan='4' style='text-align:center;'>Please select filters to view data.</td></tr>");
        return;
    }
    ...
    $.ajax({
        type: "POST",
        dataType: 'json',
        url: url_fillChart2, // Ensure this URL is correct for table data
        data: { startDate: startDate, endDate: endDate, plantLocation: plantLocation },
        success: function (response) {
            console.log("Server response:", response);
            ...
            if (!Array.isArray(dataArray)) {
                console.error("Invalid response format. Expected an array but received:", typeof response);
                $("#myTable").html("<tr><td colspan='4' style='text-align:center;'>Invalid data format</td></tr>");
                return;
            }

            if (dataArray.length === 0) {
                console.warn("No data available for table rendering.");
                $("#myTable").html("<tr><td colspan='4' style='text-align:center;'>No data available</td></tr>");
                return;
            }

            // Proceed with rendering
            renderTableRows(dataArray.slice(0, 10));
            renderPaginationControls(Math.ceil(dataArray.length / 10), dataArray, 10);
        },


    });

    const filteredData = filterTableData(raw_data); // Ensure this function is robust
    loadTablePage(1, filteredData);
}


function renderTableRows(data) {
    $("#myTable").empty(); // Clear existing rows

    if (!data || data.length === 0) {
        console.warn("No rows to render in the table.");
        $("#myTable").html("<tr><td colspan='4' style='text-align:center;'>No data available</td></tr>");
        return;
    }

    data.forEach(element => {
        const issueDate = element.issue_date
            ? Unix_timestamp(element.issue_date.split('(')[1].split(')')[0], 'md')
            : "Unknown Date";
        const duration = element.DateDiff
            ? (element.DateDiff / 60).toFixed(1) + " Hrs"
            : "N/A";
        const desc = element.material_id || element.machine_machine_id || "No Description";
        const responsiblePerson = element.Name || "N/A";

        createRow(`${issueDate}<br>(${duration})`, element.issue, desc, responsiblePerson);
    });
}

function createRow(issueDate, issue, description, responsiblePerson) {
    var tableRow = document.createElement("tr");
    console.log("create raw");

    // Issue Date
    var tableData = document.createElement("td");
    tableData.innerHTML = issueDate;
    tableData.style.whiteSpace = "nowrap"; // Prevent text wrapping
    tableRow.appendChild(tableData);

    // Issue
    var tableData2 = document.createElement("td");
    tableData2.innerHTML = issue || "Unknown Issue";
    tableData2.title = issue; // Tooltip for long text
    tableRow.appendChild(tableData2);

    // Description
    var tableData3 = document.createElement("td");
    tableData3.innerHTML = description || "No Description";
    tableData3.title = description; // Tooltip for long text
    tableRow.appendChild(tableData3);

    // Responsible Person
    var tableData4 = document.createElement("td");
    tableData4.innerHTML = responsiblePerson || "N/A";
...
function renderPaginationControls(totalPages, data, rowsPerPage) {
    if (!data || data.length === 0 || totalPages <= 1) {
        console.log("No pagination required.");
        return;
    }
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        document.getElementById("paginationControls").appendChild(pageButton);
    }
}
...
function filterByDate() {
    ...
    if (!startDate || !endDate) {
        console.warn("No date range selected. Skipping table initialization.");
        return;
    }

    createTable(); // Call only if inputs are valid
    google.setOnLoadCallback(drawChart1);
    google.setOnLoadCallback(drawChart2);
    google.setOnLoadCallback(drawChart3);
    numberOfIssues();
